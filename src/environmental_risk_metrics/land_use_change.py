...
ESA_LAND_COVER_CLASS_CONVERSION_DICT = {
    0: "No data",
    10: "Cropland, rainfed",
...
ESRI_LAND_COVER_CLASS_CONVERSION_DICT = {
    0: "No Data",
    1: "Water",
    2: "Trees",
...
GLAD_LAND_COVER_CLASS_CONVERSION_DICT = {
...
def map_esa_to_esri_classes() -> Optional[int]:
    ...
    for key, value in mapping.items():
        if value is not None:
            NEW_ESA_CLASS_MAPPING[key] = ESRI_LAND_COVER_CLASS_CONVERSION_DICT[value]
...
def map_glad_to_esri_classes() -> Optional[int]:
    ...
    for key, value in GLAD_TO_CLASSES.items():
        if value is not None:
            NEW_GLAD_CLASS_MAPPING[key] = ESRI_LAND_COVER_CLASS_CONVERSION_DICT[value]
...
class BaseLandCover:
    ...
    name: str
    band_name: str
    _class_conversion_dict: Dict[int, str]

    def get_items(self, start_date: str, end_date: str, polygon: dict) -> List[Item]:
    ...
    def load_xarray(
        ...
        logger.info(f"Loading {self.collections} data at {resolution}m resolution")
        items = self.get_items(
            start_date=start_date, end_date=end_date, polygon=polygon
        )
    ...
    def get_class_conversion_dict(self) -> Dict[int, str]:
        return self._class_conversion_dict
    ...
    def get_xarray_with_class_names(
        ...
        return ds.assign_coords(
            **{self.band_name: ds[self.band_name].map(self._class_conversion_dict)}
        )
    ...
    def get_land_use_class_percentages(
        ...
        clipped_data = images.rio.write_crs(crs).rio.clip(
            [polygon["geometry"]], polygon_crs, all_touched=all_touched
        )
...
class EsaLandCover(BaseLandCover):
    def __init__(self, use_esri_classes: bool = False) -> None:
        super().__init__(
            ...
            _class_conversion_dict=ESA_LAND_COVER_CLASS_CONVERSION_DICT
...
class EsriLandCover(BaseLandCover):
    def __init__(self) -> None:
        super().__init__(
            ...
            name="Esri Land Use",
            band_name="data",
            _class_conversion_dict=ESRI_LAND_COVER_CLASS_CONVERSION_DICT,
        )
...
class OpenLandMapLandCover(BaseLandCover):
    def __init__(self, use_esri_classes: bool = False) -> None:
        super().__init__(
            ...
            band_name="data",
            _class_conversion_dict=map_glad_to_esri_classes()
            if use_esri_classes
            else GLAD_LAND_COVER_CLASS_CONVERSION_DICT,
        )
    ...
    def load_xarray(
        ...
        data_arrays = []
        minx, miny, maxx, maxy = gpd.GeoDataFrame.from_features([polygon]).bounds.iloc[
            0
        ]
        for year in available_years:
            ...
            da = da.rio.write_crs(polygon_crs).rio.clip_box(
    ...
    def get_land_use_class_percentages(
        ...
        ds = self.load_xarray(start_date, end_date, polygon, polygon_crs, resolution)
        clipped_data = ds.rio.write_crs(polygon_crs).rio.clip(
            [polygon["geometry"]], polygon_crs, all_touched=all_touched
        )
